...
data_imputed['Inspection Frequency'] = data_imputed['Inspection Frequency'].round().astype(int)
...
for region in regions:
    region_dfs[region] = df_final[df_final['Region'] == region]
    print(f"Rows for {region}: {len(region_dfs[region])}")

# --- Save Each Region to a Separate CSV ---
print("Saving region-specific CSVs...")
for region, region_df in region_dfs.items():
    output_file = f"dam_data_imputed_{region.lower()}.csv"
    try:
...
# Display sample of each region's DataFrame
print("\nSample of each region's DataFrame:")
for region, region_df in region_dfs.items():
    print(f"\n{region}:")
    print(region_df.head())

# Display final info of the full DataFrame
print("\nFinal Full DataFrame Info:")
print(df_final.info())